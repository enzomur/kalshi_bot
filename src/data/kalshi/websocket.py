...
class WebSocketManager:
    ...
    def __init__(
        ...
        Args:
            ...
            reconnect_delay: Base delay between reconnect attempts
    ...
    async def _handle_disconnect(self) -> None:
        ...
        while (
            self._state.reconnect_attempts < self._max_reconnects and not self._shutdown
        ):
            self._state.reconnect_attempts += 1
            delay = self._reconnect_delay * (2 ** (self._state.reconnect_attempts - 1))
            delay = min(delay, 60)

            logger.info(
                f"Attempting reconnect {self._state.reconnect_attempts}/"
                f"{self._max_reconnects} in {delay}s"
            )
    ...
    async def _handle_message(self, data: dict[str, Any]) -> None:
        ...
        for sub_id, subscription in self._state.subscriptions.items():
            if subscription.callback:
                ...
                if (
                    subscription.market_ticker is None
                    or ticker == subscription.market_ticker
                ):
                    try:
