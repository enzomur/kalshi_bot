...
This client provides data fetching and order management capabilities.
It should be used by the Risk Engine and Brokers, NOT directly by strategies.
"""
...
class KalshiClient:
    ...
    This client handles all communication with the Kalshi API including:
    - Market data fetching
    - Order management
    - Account information

    The client is NOT coupled to execution logic - that belongs in the brokers.
    """
    ...
    def __init__(
        ...
        Args:
            api_base_url: Base URL for Kalshi API
    ...
    async def _request(
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                logger.error(
                    f"API {response.status_code} error on {endpoint}: {error_body}"
                )
                raise APIError(
    ...
    async def get_all_markets(
        ...
        while True:
            ...
            for market in markets:
                ...
                has_oi = (
                    market.open_interest >= min_open_interest
                    if min_open_interest > 0
                    else True
                )

                # If both thresholds specified, require at least one to pass
                if min_volume > 0 and min_open_interest > 0:
                    ...
                    if min_volume > 0 and not has_volume:
                        continue
                    if min_open_interest > 0 and not has_oi:
                        continue

                if require_orderbook and (
                    market.yes_bid is None and market.yes_ask is None
                ):
                    continue
    ...
    async def get_orderbook(self, ticker: str, depth: int = 10) -> OrderBook:
        ...
        def parse_levels(levels: list[list[int]]) -> list[OrderBookLevel]:
            return [
                OrderBookLevel(price=level[0], quantity=level[1]) for level in levels
            ]

        return OrderBook(
            market_ticker=ticker,
            yes_bids=parse_levels(
                orderbook.get("yes", [[]])[0] if orderbook.get("yes") else []
            ),
            yes_asks=parse_levels(
                orderbook.get("yes", [[], []])[1]
                if len(orderbook.get("yes", [])) > 1
                else []
            ),
            no_bids=parse_levels(
                orderbook.get("no", [[]])[0] if orderbook.get("no") else []
            ),
            no_asks=parse_levels(
                orderbook.get("no", [[], []])[1]
                if len(orderbook.get("no", [])) > 1
                else []
            ),
        )
